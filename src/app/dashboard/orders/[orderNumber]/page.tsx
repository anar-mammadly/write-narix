...
export default async function OrderDetailPage({ params }: { params: Promise<{ orderNumber: string }> }) {
  ...
  const { data: order } = await supabase
    .from("orders")
    .select(
      "id, order_number, subject, topic, description, university, college, base_price, discount_source, discount_percentage, discount_amount, final_price, reviewed_price, original_price, paid_amount, remaining_amount, is_fully_paid, created_at, order_statuses(name, color), services(name), academic_levels(name), deadline_options(label)"
    )
    ...
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6">
      ...
      <div className={`mt-6 grid gap-2 rounded-xl border border-border bg-card p-4 text-sm sm:gap-4 sm:p-5 ${order.reviewed_price != null ? "grid-cols-2 sm:grid-cols-4" : "grid-cols-3"}`}>
        ...
        {order.reviewed_price != null && (
          <div><p className="text-muted-foreground">{dict.admin.workspace.reviewedPriceTitle}</p><p className="mt-1 font-medium text-success">{Number(order.reviewed_price).toFixed(2)} {dict.common.currency}</p></div>
        )}
