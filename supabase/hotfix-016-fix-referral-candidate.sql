-- ---------------------------------------------------------------------------
-- hotfix-016: re-affirm calculate_price() exactly as committed.
--
-- Diagnosed live: approving a referral (approve_referral_with_percentage)
-- updated referral_benefits correctly (status='approved', percentage set,
-- not expired) but the order's final_price never changed — calculate_price()
-- never produced a 'referral' candidate even when every condition on the
-- benefit row was verified to match directly via SQL. The deployed function
-- body had drifted from what's committed in full-setup.sql. This is a
-- byte-for-byte CREATE OR REPLACE of the same 12-arg signature already live
-- (proven live by the working promo-approval path), so it's a pure
-- reset — no grants change, no signature change, nothing else touched.
-- ---------------------------------------------------------------------------
