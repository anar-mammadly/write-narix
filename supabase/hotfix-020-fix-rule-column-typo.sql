-- ---------------------------------------------------------------------------
-- hotfix-020: fixes a mistake in hotfix-019 itself.
--
-- hotfix-019 changed every "<row> IS NOT NULL" composite check to
-- "<row>.id IS NOT NULL" — correct everywhere except one: the very first
-- check, `rule service_pricing_rules`, was accidentally changed too, and
-- that table's primary key column is `service_id`, not `id`. This broke
-- calculate_price() entirely ("record rule has no field id"). This
-- byte-for-byte re-applies hotfix-019 with that one line corrected.
-- ---------------------------------------------------------------------------
...
begin
  ...
  if rule.service_id is null then
